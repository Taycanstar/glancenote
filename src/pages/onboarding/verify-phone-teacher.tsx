...
import { loginWithoutPassword } from "@/src/store/user";

const VerifyPhoneTeacher: React.FC & {
  ...
  const handleResendCode = async () => {
    if (email) {
      try {
        const response = await fetch(`${LOCAL}/api/resend-teacher-code/`, {
          method: "POST",
          headers: {
  ...
  const handleSubmit = async () => {
    if (email) {
      try {
        const response = await fetch(`${LOCAL}/api/confirm-teacher-ph/`, {
          method: "POST",
          headers: {
            "Content-Type": "application/json",
            // 'X-CSRFToken': csrfToken // Include this if CSRF is used.
...
VerifyPhoneTeacher.Layout = LogoLayout;
export default VerifyPhoneTeacher;
